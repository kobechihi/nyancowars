...
def reverse_calculate_debuff(original_power, debuff_power, original_level, disadvantage, kakin):
   # デバフ後の戦力から逆算してKILL数を求める
   if kakin:
       debuff_power /= 1.13
   if disadvantage:
       debuff_power /= 1.25
   level_decrease = original_level * (1 - debuff_power / original_power)
   kill_count = level_decrease / (original_level * 0.0024)
   return round(kill_count)
def get_opponent_suggestions(my_team, opponent_team):
   suggestions = []
   for _, opponent in opponent_team.iterrows():
       best_matchups = []
       for _, ally in my_team.iterrows():
           # 属性相性を考慮（火→木→水→火）
           advantage = False
           if (ally['属性'] == '火' and opponent['属性'] == '木') or \
              (ally['属性'] == '木' and opponent['属性'] == '水') or \
              (ally['属性'] == '水' and opponent['属性'] == '火'):
               advantage = True
           kill_count = reverse_calculate_debuff(
               ally['最高戦力'], opponent['最高戦力'],
               200, not advantage, True  # レベル200、有利属性の場合はFalse、課金ありと仮定
           )
           best_matchups.append((ally['名前'], kill_count))
       best_matchups.sort(key=lambda x: x[1])  # KILL数が少ない順にソート
       suggestions.append((opponent['名前'], best_matchups[:3]))
   return suggestions
...
def main():
   ...
   if not st.session_state.my_team.empty and not st.session_state.opponent_team.empty:
       suggestions = get_opponent_suggestions(st.session_state.my_team, st.session_state.opponent_team)
       for opponent_name, matchups in suggestions:
           st.subheader(f"{opponent_name}に対する推奨メンバー")
           for ally_name, kill_count in matchups:
               st.write(f"{ally_name} (推定KILL数: {kill_count})")
   else:
       st.write("自チームと対戦相手チームの両方にメンバーを登録してください。")
